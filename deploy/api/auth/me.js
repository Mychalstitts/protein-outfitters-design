// /api/auth/me
//   GET   → { user } — current user or null
//   PATCH → { user }  — update name / email / phone / zip / avatar_url for the signed-in user
//
// Role is NOT updatable from this endpoint — that requires admin elevation
// to prevent a user from self-promoting to admin. Same applies to id and any
// internal columns. We strictly allow-list the fields a user can edit.
import { sql, currentUser, json, err } from '../_lib/db.js';

export const config = { runtime: 'edge' };

const EMAIL_RE = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
// Generous bounds — we don't want to fight reasonable inputs.
...
export default async function handler(req) {
  ...
  if (req.method === 'PATCH') {
    ...
    // Allow-list every editable field. Anything else in the body is ignored.
    const patch = {};
    ...
    // Empty patch is a no-op success — frontend may call this defensively.
    if (Object.keys(patch).length === 0) return json({ user });

    // Validate email format (only when present).
    if ('email' in patch && patch.email && !EMAIL_RE.test(patch.email)) {
      return err(400, 'Invalid email format');
    }

    // If email changed, ensure it's not already taken by someone else.
    ...
    // Build dynamic UPDATE. Each tagged-template field gets its own placeholder.
    // neon's `sql` tag doesn't support dynamic columns inside one template,
    // so we run separate statements (cheap — at most 5 trivial UPDATEs in practice).
